...
def plot_one(method, color, x, y, linestyle="-", marker='d'):
    try:
        ...
        x = np.array(data[4:8, x])
        # x = np.log(x)
        y = np.array(data[4:8, y])
        method_name = method.replace("norm_rq_kmeans", "NE-RQ")
        method_name = method_name.replace("norm_rq_partial_kmeans", "Non-Uniform NE-RQ")
...
def plot_(x, y):
    ...
    plot_one('norm_rq_partial_kmeans', 'red', x, y, ':', 's')
    plot_one('norm_rq_kmeans',  'black',    x, y, '-', 'o')
